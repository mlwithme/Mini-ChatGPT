...
def _load_messages_from_checkpoint(agent, args, thread_id: str) -> list[dict[str, str]]:
...
def ensure_session_state(agent, args) -> None:
    ...
    if thread_id not in st.session_state.messages_by_thread:
        st.session_state.messages_by_thread[thread_id] = _load_messages_from_checkpoint(agent, args, thread_id)
